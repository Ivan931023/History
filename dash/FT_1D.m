...
% for t = 1:iteration
%     DOEphase = exp(1j * DOE);
%     y_focal = fft(initial_profile .* DOEphase);
...
%     y_SLM = ifft(Target .* exp_focal_phase);
...
%     DOEphase = exp(1j * DOE);
%     y_focal = sqrt((dx/(1j*wl*z)))*exp((1j*pi*ox.^2)/(wl*z)).*...
%               (initial_profile.*DOEphase.*exp((1j*pi*x.^2)/(wl*z))* ...
%               exp((-1j*2*pi)/(wl*z) * transpose(x)*ox ));
%     y_focal_phase = angle(y_focal);
%     y_focal_intensity = abs(y_focal).^2;
%     y_focal_intensity = y_focal_intensity / max(y_focal_intensity,[],"all");
...
%     y_SLM = conj(sqrt((dx/(1j*wl*z))))*exp((1j*pi*x.^2)/(wl*z)).*...
%             (Target.*exp_focal_phase.*exp((1j*pi*ox.^2)/(wl*z))* ...
%             exp((1j*2*pi)/(wl*z) * transpose(ox)*x ));
%     y_SLM_phase = angle(y_SLM);
%     DOE = y_SLM_phase;
% 
%     t
