...
% data_zoomin = data_zoomin(v-range:v+range,h-range:h+range);
data_zoomin = data_zoomin / max(data_zoomin(:));

% c = range+1;
% up_index = c-(height-1)/2;
% down_index = c+(height-1)/2;
% left_index = c-(width-1)/2;
% right_index = c+(width-1)/2;
...
for i = 1:times
    ...
    % if i > 1
    %     path = pwd;
    %     cd('G:\Other computers\yan\Files\Lab\Experiment_records\Metropolis_data_test')
    %     f10 = figure;
    %     imagesc(data_zoomin1,[0 1])
    %     axis image
    %     colormap turbo
    %     colorbar
    %     title(sprintf('i = %d, Z(%g) : %g -> %g',i-1,samples(n),old_z,new_z))
    %     xlabel(sprintf('2.2\\mum/pixel'));ylabel(sprintf('2.2\\mum/pixel'));
    %     exportgraphics(f10,sprintf('i_%d.png',i-1))
    %     writematrix(data_zoomin1,sprintf('data_%d.csv',i-1))
    %     Generate_GIF(f10,'Test.gif',0.01,i-1)
    %     cd(path)
    % end
