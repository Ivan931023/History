...
close all; move_image_on_screen(move_x, move_y, pixel, phase_origin); pause(0.1)
image_origin = 0;
...
close all; move_image_on_screen(move_x, move_y, pixel, phase0); pause(0.1)
image = 0;
for ii = 1:snapshot_times
    start(vid); data = getdata(vid,1); data = double(data); stop(vid)
    image = image + data;
...
for i = 1:times
    tic
    if i < times1
        samples = samples1; step = step1; w0 = 1; w1 = 0;
        if i == 1; T = T1; beta = 1/T; end
    elseif i >= times1 && i < times2
        samples = samples2; step = step2; w0 = 0.9; w1 = 1-w0;
        if i == times1; T = T2; beta = 1/T; end
    else
        samples = samples3; step = step3; w0 = 0.6; w1 = 1-w0;
    ...
    if i > 1
        ...
        close all; move_image_on_screen(move_x, move_y, pixel, phase1); pause(0.1)
        image1 = 0;
        for ii = 1:snapshot_times
        ...
        criteria0 = min(1,P01/P00); criteria1 = min(1,P11/P10);
    ...
    RMS00_array(i) = RMS00; RMS10_array(i) = RMS10;
...
exportgraphics(figure,append('Best_CCD_image_',Get_time(),'.png'))
