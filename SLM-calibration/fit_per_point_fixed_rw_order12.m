% fit_per_point_fixed_rw_order12.m
% 固定 r,w；對以中心(541,541)為中心的 713x713 ROI 進行精細擬合，聯合 Order1+Order2 χ²
clc; clear; close all;
tic;
...
a2pi_range  = 232:2:242;
alpha_range = (-0.5:0.1:0.4) * pi;
...
roi_half = (713-1)/2;
cy = 541; cx = 541;
r1 = cy - roi_half; r2 = cy + roi_half;
c1 = cx - roi_half; c2 = cx + roi_half;
assert(r1>=1 && c1>=1 && r2<=Ny && c2<=Nx, 'ROI 超出邊界。');

exp1R = exp1(r1:r2, c1:c2, :);
